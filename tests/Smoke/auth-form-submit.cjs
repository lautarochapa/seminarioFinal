...
(async () => {
    ...
    busy.calls[0].resolve(session);
    await flush();
    busy.submit();
    assert.equal(busy.calls.length, 1, 'The confirmed login payload avoids another session lookup.');
    assert.equal(busy.window.location.href, '/web');
    busy.submit(); await flush();
    assert.equal(busy.calls.length, 1, 'Keep locked until navigation finishes.');
    assert.ok(busy.buttons.every(button => button.disabled));
    ...
    retry.calls[1].resolve(session); await flush();
    retry.submit(); await flush();
