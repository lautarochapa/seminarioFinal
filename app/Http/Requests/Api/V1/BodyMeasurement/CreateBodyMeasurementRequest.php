...
class CreateBodyMeasurementRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // At least one body measurement required
            $hasMeasurement = $this->has('weight_kg')
                || $this->has('waist_cm')
                || $this->has('blood_pressure_systolic')
                || $this->has('glucose_level');

            if (!$hasMeasurement) {
                $validator->errors()->add('measurement', 'Debe proporcionar al menos una medición corporal.');
            }
    ...
    public function validated()
    {
        ...
        if (array_key_exists('measured_at', $data)) {
            ...
            $data['measurement_date'] = now()->format('Y-m-d');
        }
