...
import type { BudgetProjection } from '@/types/budget';
...
export function useBudgetProjection(groupId: number | null, budgetId: number | null) {
  const [data, setData] = useState<BudgetProjection | null>(null);
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    setError(null);
    budgetsApi.projection(groupId, budgetId)
