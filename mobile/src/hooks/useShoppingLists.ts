...
import type { ShoppingList, ShoppingListFilters } from '@/types/shopping';
...
export function useShoppingLists(groupId: number | null) {
  const [state, setState] = useState<State>({ data: [], meta: DEFAULT_META, loading: false, loadingMore: false, error: null });
  const [filters, setFiltersRaw] = useState<ShoppingListFilters>({});
  const pageRef = useRef(1);
  const version = useRef(0);

  const fetch = useCallback((gid: number, f: ShoppingListFilters, page: number, append: boolean) => {
    const v = ++version.current;
    setState((s) => ({ ...s, loading: !append, loadingMore: append, error: null }));
    shoppingListsApi.list(gid, { ...f, page, per_page: 20 })
      .then((res) => {
  ...
  const setFilters = useCallback((f: ShoppingListFilters) => { pageRef.current = 1; setFiltersRaw(f); }, []);
