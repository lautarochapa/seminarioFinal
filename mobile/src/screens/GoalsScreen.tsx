...
export function GoalsScreen() {
  ...
  const goBack = fromOnboarding ? returnToOnboarding : goBackOrHome;
  ...
  }, [fromOnboarding, returnToOnboarding]));
  ...
  async function save() {
    ...
    try {
      ...
      setMessage('Tus objetivos se guardaron correctamente.');
      refresh();
      if (fromOnboarding) {
        Alert.alert('Datos guardados', 'Podés modificarlos más adelante desde tu perfil.', [
          { text: 'Seguir editando', style: 'cancel' },
          { text: 'Continuar', onPress: returnToOnboarding },
        ]);
      }
  ...
  if (error && !data) return <View style={styles.fill}><AppHeader title="Mis objetivos" showBack onBack={goBack} /><ErrorState message={friendlyMessage(error)} traceId={error.traceId} onRetry={refresh} type="server" /></View>;
  return <KeyboardAvoidingView style={styles.fill} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><AppHeader title="Mis objetivos" showBack onBack={goBack} /><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <Block title="Comidas por día"><AppInput label="Cantidad de comidas por día" value={form.meals} onChangeText={(meals) => setForm((v) => ({ ...v, meals }))} keyboardType="number-pad" /></Block>
