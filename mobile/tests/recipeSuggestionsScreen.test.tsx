...
const mockPush = jest.fn(); const mockAvailable = jest.fn(); const mockAlmost = jest.fn(); const mockSoon = jest.fn();
...
jest.mock('../src/api/endpoints', () => ({ recipeSuggestionsApi: { available: (...args: unknown[]) => mockAvailable(...args), almostAvailable: (...args: unknown[]) => mockAlmost(...args), byExpiringStock: (...args: unknown[]) => mockSoon(...args) } }));
describe('RecipeSuggestionsScreen', () => {
  beforeEach(() => { jest.clearAllMocks(); mockAvailable.mockResolvedValue({ data: [{ recipe: { id: 10, name: 'Arroz con pollo' } }] }); mockAlmost.mockResolvedValue({ data: [{ recipe: { id: 11, name: 'Tarta' }, missing_ingredients_count: 2 }] }); mockSoon.mockResolvedValue({ data: [{ recipe: { id: 12, name: 'Panqueques' } }] }); });
  it('shows separated sections and opens recipe', async () => { const screen = await render(<RecipeSuggestionsScreen />); await waitFor(() => expect(screen.getByText('Arroz con pollo')).toBeTruthy()); expect(screen.getByText('Para cocinar ahora')).toBeTruthy(); expect(screen.getByText('Te falta poco')).toBeTruthy(); expect(screen.getByText('Te faltan 2 ingredientes')).toBeTruthy(); expect(screen.getByText('Conviene cocinar pronto')).toBeTruthy(); expect(screen.getByText('Usá ingredientes por vencer')).toBeTruthy(); fireEvent.press(screen.getByText('Arroz con pollo')); expect(mockPush).toHaveBeenCalledWith({ pathname: '/(app)/recipes/[id]', params: { id: '10' } }); });
  it('respects the active group', async () => { await render(<RecipeSuggestionsScreen />); await waitFor(() => expect(mockAvailable).toHaveBeenCalledWith(1)); expect(mockAlmost).toHaveBeenCalledWith(1); expect(mockSoon).toHaveBeenCalledWith(1); });
});
